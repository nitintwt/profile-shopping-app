...
export default function ProductCard({productId , imageLink , name , price , handleAddToCart}) {
  ...
  useEffect(()=>{
    const checkProductInCart = async ()=>{
      try {
        const check = await axios.get(`${import.meta.env.VITE_AWS_API}/api/v1/users/checkProductInCart?userId=${cookies?.userData?._id}&productId=${productId}`, {withCredentials: true,})
